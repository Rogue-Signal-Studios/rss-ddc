...
static void apply_prior(RSSDDCCharacterization *characterization) {
...
static void test_assemble_matches_and_normalizes_profile_controls(void) {
    ...
    assert(rss_ddc_characterization_structured_match(characterization) ==
    ...
    assert(rss_ddc_monitor_knowledge_route_count(
               rss_ddc_characterization_discovered_knowledge(characterization)) == 0);
    apply_prior(characterization);
...
static void test_assemble_retains_competing_facts(void) {
    ...
    assert(rss_ddc_characterization_assemble(characterization, &display, NULL, store) == RSS_DDC_OK);
    assert(rss_ddc_monitor_knowledge_route_count(
               rss_ddc_characterization_discovered_knowledge(characterization)) == 1);
    apply_prior(characterization);
    assert(rss_ddc_monitor_knowledge_route_count(rss_ddc_characterization_knowledge(characterization)) ==
           4);
    assert(rss_ddc_characterization_resolve(characterization, "brightness", &resolution) == RSS_DDC_OK);
...
static void test_assemble_profile_merge_overflow_is_explicit(void) {
    ...
    assert(rss_ddc_characterization_profile_status(characterization) ==
           RSS_DDC_CHARACTERIZATION_PROFILE_MATCHED);
    assert(rss_ddc_characterization_display(characterization) != NULL);
    assert(rss_ddc_characterization_augment_with_prior(characterization) == RSS_DDC_ERROR_PROFILE_CONFLICT);
    assert(rss_ddc_monitor_knowledge_route_count(rss_ddc_characterization_knowledge(characterization)) ==
           128);
    rss_ddc_monitor_knowledge_destroy(batch);
    rss_ddc_profile_store_destroy(store);
...
static void test_passive_mccs_preserves_profile_authorization(void) {
    ...
    assert(route_with_semantic(rss_ddc_characterization_discovered_knowledge(characterization),
                               "display.brightness")
               ->provenance.fact_kind == RSS_DDC_KNOWLEDGE_FACT_DECLARED);
    apply_prior(characterization);
    knowledge = rss_ddc_characterization_knowledge(characterization);
    for (size_t index = 0; index < rss_ddc_monitor_knowledge_route_count(knowledge); ++index) {
        const RSSDDCKnowledgeRoute *route = rss_ddc_monitor_knowledge_route_at(knowledge, index);
...
static void test_passive_mccs_malformed_and_failed_preserve_state(void) {
    ...
    assert(rss_ddc_profile_store_load_pack_data(store, pack, strlen(pack)) == RSS_DDC_OK);
    assert(rss_ddc_characterization_assemble(characterization, &display, NULL, store) == RSS_DDC_OK);
    before = rss_ddc_monitor_knowledge_route_count(
    ...
    assert(rss_ddc_characterization_mccs(characterization) == NULL);
    assert(rss_ddc_monitor_knowledge_route_count(
               rss_ddc_characterization_discovered_knowledge(characterization)) == before);
    assert(rss_ddc_characterization_display(characterization) != NULL);
    assert(rss_ddc_characterization_profile_status(characterization) ==
           RSS_DDC_CHARACTERIZATION_PROFILE_MATCHED);
...
static void test_quick_probe_profile_declared_observed_coexist(void) {
    ...
    assert(route_with_semantic(rss_ddc_characterization_discovered_knowledge(characterization),
                               "display.brightness")
               ->provenance.fact_kind != RSS_DDC_KNOWLEDGE_FACT_PROFILE);
    apply_prior(characterization);
    knowledge = rss_ddc_characterization_knowledge(characterization);
...
static void test_quick_probe_failure_preserves_prior_knowledge(void) {
    ...
    before = rss_ddc_monitor_knowledge_route_count(
        rss_ddc_characterization_discovered_knowledge(characterization));
    assert(before > 0);
    assert(count_fact_kind(rss_ddc_characterization_discovered_knowledge(characterization),
                           "display.brightness", RSS_DDC_KNOWLEDGE_FACT_PROFILE) == 0);
...
static void test_sufficiency_actionable_profile_without_live_values(void) {
    ...
    const RSSDDCKnowledgeRoute *current = (const RSSDDCKnowledgeRoute *)0x1;
    RSSDDCCharacterizationSufficiencyResult discovery = {0};
    assert(rss_ddc_characterization_structured_match(characterization) ==
           RSS_DDC_CHARACTERIZATION_STRUCTURED_COMPLETE);
    assert(rss_ddc_characterization_discovery_sufficiency(characterization, &discovery) == RSS_DDC_OK);
    assert(discovery.status == RSS_DDC_CHARACTERIZATION_SUFFICIENCY_INSUFFICIENT);
    apply_prior(characterization);
    assert(rss_ddc_characterization_sufficiency(characterization, &result) == RSS_DDC_OK);
...
static void test_sufficiency_validated_input_without_quick_0x60(void) {
    ...
    assert(route_with_kind(rss_ddc_characterization_discovered_knowledge(characterization),
                           "inputs.switching", RSS_DDC_KNOWLEDGE_FACT_OBSERVED) == NULL);
    assert(rss_ddc_characterization_discovery_sufficiency(characterization, &result) == RSS_DDC_OK);
    assert(result.status == RSS_DDC_CHARACTERIZATION_SUFFICIENCY_INSUFFICIENT);
    assert(result.extended_recommended);
    apply_prior(characterization);
    assert(rss_ddc_characterization_sufficiency(characterization, &result) == RSS_DDC_OK);
...
static void test_extended_not_recommended_is_not_run(void) {
    RSSDDCDisplay display = slice2_display();
    RSSDDCCharacterization *characterization = slice5_assembled(&display, slice5_actionable_pack_dcpdp13());
    size_t before = rss_ddc_monitor_knowledge_route_count(
        rss_ddc_characterization_discovered_knowledge(characterization));
    RSSDDCCharacterizationSufficiencyResult discovery = {0};
    RSSDDCCharacterizationSufficiencyResult sufficiency = {0};
    assert(rss_ddc_characterization_discovery_sufficiency(characterization, &discovery) == RSS_DDC_OK);
    assert(discovery.status == RSS_DDC_CHARACTERIZATION_SUFFICIENCY_INSUFFICIENT);
    assert(discovery.extended_recommended);
    apply_prior(characterization);
...
static void test_extended_priority_beats_capacity(void) {
    ...
    size_t before = 0;
    for (unsigned index = 0; index < 126; ++index) {
        char route_id[RSS_DDC_PROFILE_ID_MAX] = {};
        (void)snprintf(route_id, sizeof(route_id), "fill-%u", index);
        RSSDDCKnowledgeRoute route =
            make_route("display.rgb.red_gain", "fill", RSS_DDC_PROFILE_SOURCE_RESEARCH,
                       RSS_DDC_PROFILE_CONFIDENCE_OBSERVED, RSS_DDC_KNOWLEDGE_FACT_OBSERVED, route_id,
                       (uint16_t)index, RSS_DDC_KNOWLEDGE_VALUE_UNSIGNED, index, true, false, false);
        assert(rss_ddc_monitor_knowledge_add_route(batch, &route) == RSS_DDC_OK);
    }
...
static RSSDDCProfileStore *store_from_pack(const char *pack) {
    RSSDDCProfileStore *store = rss_ddc_profile_store_create();
    assert(store != NULL);
    assert(rss_ddc_profile_store_load_pack_data(store, pack, strlen(pack)) == RSS_DDC_OK);
    return store;
...
static void test_profile_free_lg_does_not_acquire_lg_alt(void) {
    RSSDDCCharacterization *characterization = rss_ddc_characterization_create();
    RSSDDCDisplay display = lg_hdr_qhd_display();
    assert(characterization != NULL);
    assert(rss_ddc_characterization_assemble(characterization, &display, NULL, NULL) == RSS_DDC_OK);
    assert(rss_ddc_characterization_structured_match(characterization) ==
           RSS_DDC_CHARACTERIZATION_STRUCTURED_NONE);
    assert(route_with_kind_and_route_kind(rss_ddc_characterization_knowledge(characterization),
                                          "inputs.switching", RSS_DDC_KNOWLEDGE_ROUTE_LG_ALT_INPUT) == NULL);
    rss_ddc_characterization_destroy(characterization);
}

static void test_builtin_lg_profile_adds_lg_alt_authority(void) {
    RSSDDCCharacterization *characterization = rss_ddc_characterization_create();
    RSSDDCDisplay display = lg_hdr_qhd_display();
    RSSDDCProfileStore *store = rss_ddc_profile_store_create();
    RSSDDCMonitorKnowledgeResolution *resolution = NULL;
    ...
    assert(rss_ddc_characterization_structured_match(characterization) ==
           RSS_DDC_CHARACTERIZATION_STRUCTURED_PARTIAL);
    assert(route_with_kind_and_route_kind(rss_ddc_characterization_discovered_knowledge(characterization),
                                          "inputs.switching", RSS_DDC_KNOWLEDGE_ROUTE_LG_ALT_INPUT) == NULL);
    apply_prior(characterization);
    ...
    assert(lg_alt->provenance.fact_kind == RSS_DDC_KNOWLEDGE_FACT_PROFILE);
    assert(lg_alt->address == 0xf4);
...
static void test_odyssey_gains_no_model_specific_authority(void) {
    RSSDDCCharacterization *characterization = rss_ddc_characterization_create();
    RSSDDCDisplay display = odyssey_g75f_display();
    RSSDDCProfileStore *store = rss_ddc_profile_store_create();
    assert(characterization != NULL && store != NULL);
    assert(rss_ddc_profile_store_load_builtin(store) == RSS_DDC_OK);
    assert(rss_ddc_characterization_assemble(characterization, &display, NULL, store) == RSS_DDC_OK);
...
static void test_observed_vcp60_never_creates_lg_alt(void) {
    RSSDDCCharacterization *characterization = rss_ddc_characterization_create();
    RSSDDCDisplay display = lg_hdr_qhd_display();
    Slice6MockGet mock = {0};
    RSSDDCProbe *probe = NULL;
...
static void test_complete_match_skips_default_probing(void) {
    Slice7Harness harness = slice7_harness(slice2_display());
    ...
    assert(rss_ddc_characterization_structured_match(result) == RSS_DDC_CHARACTERIZATION_STRUCTURED_COMPLETE);
    assert(!rss_ddc_characterization_discovery_performed(result));
    assert(rss_ddc_characterization_prior_augmented(result));
    assert(count_fact_kind(rss_ddc_characterization_knowledge(result), "display.brightness",
                           RSS_DDC_KNOWLEDGE_FACT_PROFILE) == 1);
    assert(count_fact_kind(rss_ddc_characterization_discovered_knowledge(result), "display.brightness",
                           RSS_DDC_KNOWLEDGE_FACT_PROFILE) == 0);
...
static void test_complete_match_skips_passive_mccs(void) {
    Slice7Harness harness = slice7_harness(slice2_display());
    RSSDDCProfileStore *store = store_from_pack(slice5_actionable_pack_dcpdp13());
    RSSDDCCharacterization *result = NULL;
    RSSDDCCharacterizeOptions options = {.mode = RSS_DDC_CHARACTERIZE_MODE_PASSIVE};
...
static void test_partial_match_does_not_skip_default(void) {
...
static void test_deep_discovers_with_complete_profile(void) {
    Slice7Harness harness = slice7_harness(slice2_display());
    RSSDDCProfileStore *store = store_from_pack(slice5_actionable_pack_dcpdp13());
    RSSDDCCharacterization *result = NULL;
    RSSDDCCharacterizeOptions options = {.mode = RSS_DDC_CHARACTERIZE_MODE_DEEP};
...
static void test_ignore_known_has_no_profile_facts(void) {
    Slice7Harness harness = slice7_harness(lg_hdr_qhd_display());
    RSSDDCProfileStore *store = rss_ddc_profile_store_create();
    RSSDDCCharacterization *result = NULL;
    RSSDDCCharacterizeOptions options = {.mode = RSS_DDC_CHARACTERIZE_MODE_DEEP,
    ...
    assert(rss_ddc_characterization_knowledge(result) ==
           rss_ddc_characterization_discovered_knowledge(result));
    assert(!rss_ddc_characterization_prior_augmented(result));
...
static void prepare_lg_discovery_harness(Slice7Harness *harness) {
...
static void test_partial_prior_is_quarantined_from_discovery(void) {
    RSSDDCCharacterization *characterization = rss_ddc_characterization_create();
    RSSDDCDisplay display = lg_hdr_qhd_display();
    RSSDDCProfileStore *store = rss_ddc_profile_store_create();
    RSSDDCCharacterizationSufficiencyResult discovery = {0};
    ...
    assert(rss_ddc_characterization_structured_match(characterization) ==
           RSS_DDC_CHARACTERIZATION_STRUCTURED_PARTIAL);
    assert(count_fact_kind(rss_ddc_characterization_discovered_knowledge(characterization),
                           "display.picture_mode", RSS_DDC_KNOWLEDGE_FACT_PROFILE) == 0);
    assert(route_with_kind_and_route_kind(rss_ddc_characterization_discovered_knowledge(characterization),
                                          "inputs.switching", RSS_DDC_KNOWLEDGE_ROUTE_LG_ALT_INPUT) == NULL);
    for (size_t index = 0; index < RSS_DDC_PROBE_QUICK_CONTROL_COUNT; ++index) {
        slice4_set_stable(&mock, index, 100, 1);
    ...
    assert(discovery.extended_recommended);
    assert(count_fact_kind(rss_ddc_characterization_discovered_knowledge(characterization),
                           "inputs.switching", RSS_DDC_KNOWLEDGE_FACT_PROFILE) == 0);
    rss_ddc_probe_destroy(probe);
    rss_ddc_characterization_destroy(characterization);
    rss_ddc_profile_store_destroy(store);
}

static void test_lg_partial_matches_profile_free_then_augments(void) {
    ...
    RSSDDCMonitorKnowledgeResolution *resolution = NULL;
    const RSSDDCKnowledgeRoute *write = NULL;
    const RSSDDCKnowledgeRoute *observed_input = NULL;
    const RSSDDCKnowledgeRoute *observed_picture = NULL;
    assert(store != NULL);
    assert(rss_ddc_profile_store_load_builtin(store) == RSS_DDC_OK);
    ...
    assert(with_profile.quick_calls == no_profile.quick_calls);
    assert(with_profile.extended_calls == no_profile.extended_calls);
    assert(with_profile.quick_calls == 1);
    assert(with_profile.extended_calls == 1);
    assert(with_profile.get_mccs_calls == no_profile.get_mccs_calls);
    assert(rss_ddc_characterization_discovery_performed(partial));
    assert(rss_ddc_characterization_discovery_performed(alien));
    assert(rss_ddc_characterization_prior_augmented(partial));
    assert(!rss_ddc_characterization_prior_augmented(alien));
    assert(rss_ddc_characterization_knowledge(alien) ==
           rss_ddc_characterization_discovered_knowledge(alien));
    assert(route_with_kind_and_route_kind(rss_ddc_characterization_discovered_knowledge(partial),
                                          "inputs.switching", RSS_DDC_KNOWLEDGE_ROUTE_LG_ALT_INPUT) == NULL);
    assert(route_with_kind_and_route_kind(rss_ddc_characterization_knowledge(partial),
                                          "inputs.switching", RSS_DDC_KNOWLEDGE_ROUTE_LG_ALT_INPUT) != NULL);
    assert(route_with_kind_and_route_kind(rss_ddc_characterization_knowledge(alien), "inputs.switching",
                                          RSS_DDC_KNOWLEDGE_ROUTE_LG_ALT_INPUT) == NULL);
    observed_input = route_with_kind(rss_ddc_characterization_discovered_knowledge(partial),
                                     "inputs.switching", RSS_DDC_KNOWLEDGE_FACT_OBSERVED);
    observed_picture = route_with_kind(rss_ddc_characterization_discovered_knowledge(partial),
                                       "display.picture_mode", RSS_DDC_KNOWLEDGE_FACT_OBSERVED);
    assert(observed_input != NULL && observed_input->address == 0x60);
    assert(!observed_input->write_authorized);
    assert(observed_picture != NULL && observed_picture->address == 0x15);
    assert(!observed_picture->write_authorized);
    assert(rss_ddc_characterization_resolve(partial, "display.picture_mode", &resolution) == RSS_DDC_OK);
    write = rss_ddc_monitor_knowledge_resolution_preferred_write(resolution);
    assert(write != NULL && write->write_authorized);
    assert(write->provenance.fact_kind == RSS_DDC_KNOWLEDGE_FACT_PROFILE);
    rss_ddc_monitor_knowledge_resolution_destroy(resolution);
...
static void test_profile_and_mccs_remain_distinct(void) {
    ...
    declared = route_with_kind(rss_ddc_characterization_knowledge(characterization), "inputs.switching",
                               RSS_DDC_KNOWLEDGE_FACT_DECLARED);
    assert(lg_alt != NULL && lg_alt->write_authorized);
    assert(declared != NULL && !declared->write_authorized && declared->address == 0x60);
    rss_ddc_characterization_destroy(characterization);
    rss_ddc_profile_store_destroy(store);
}
...
static void test_profile_update_observations_do_not_authorize_or_persist_current(void) {
    ...
    RSSDDCProfileStore *lg_profiles = rss_ddc_profile_store_create();
    assert(lg_profiles != NULL);
    assert(rss_ddc_profile_store_load_builtin(lg_profiles) == RSS_DDC_OK);
    assert(rss_ddc_characterization_assemble(lg, &lg_display, NULL, lg_profiles) == RSS_DDC_OK);
    ...
    rss_ddc_profile_store_destroy(lg_profiles);
    rss_ddc_profile_store_destroy(target);
}
...
static void assert_characterizations_equivalent(const RSSDDCCharacterization *left,
    ...
    assert(rss_ddc_characterization_next_interaction(left) ==
           rss_ddc_characterization_next_interaction(right));
    assert(rss_ddc_characterization_next_action(left) == rss_ddc_characterization_next_action(right));
}
...
static void assert_write_authority_unchanged(const RSSDDCCharacterization *characterization,
                                             bool *authorized, size_t count) {
    size_t index = 0;
    assert(rss_ddc_monitor_knowledge_route_count(rss_ddc_characterization_knowledge(characterization)) ==
           count);
    for (index = 0; index < count; ++index) {
        const RSSDDCKnowledgeRoute *route =
            rss_ddc_monitor_knowledge_route_at(rss_ddc_characterization_knowledge(characterization), index);
        assert(route != NULL);
        assert(route->write_authorized == authorized[index]);
    }
}

static void snapshot_write_authority(const RSSDDCCharacterization *characterization, bool *authorized,
                                     size_t *count) {
    size_t index = 0;
    *count = rss_ddc_monitor_knowledge_route_count(rss_ddc_characterization_knowledge(characterization));
    assert(*count <= 128);
    for (index = 0; index < *count; ++index) {
        const RSSDDCKnowledgeRoute *route =
            rss_ddc_monitor_knowledge_route_at(rss_ddc_characterization_knowledge(characterization), index);
        assert(route != NULL);
        authorized[index] = route->write_authorized;
    }
}

static void assert_no_pending_interaction(const RSSDDCCharacterization *characterization,
                                          RSSDDCCharacterizationAction expected_action) {
    RSSDDCCharacterizationInteraction first;
    RSSDDCCharacterizationInteraction second;
    RSSDDCCharacterizationStage stage = rss_ddc_characterization_stage(characterization);
    bool discovery = rss_ddc_characterization_discovery_performed(characterization);
    bool quick = rss_ddc_characterization_quick_attempted(characterization);
    bool extended = rss_ddc_characterization_extended_attempted(characterization);
    bool authorized[128];
    size_t route_count = 0;
    snapshot_write_authority(characterization, authorized, &route_count);
    memset(&first, 0xff, sizeof(first));
    memset(&second, 0xff, sizeof(second));
    assert(rss_ddc_characterization_next_interaction(characterization) ==
           RSS_DDC_CHARACTERIZATION_INTERACTION_NONE);
    assert(rss_ddc_characterization_next_action(characterization) == expected_action);
    assert(rss_ddc_characterization_interaction(characterization, &first) == RSS_DDC_OK);
    assert(first.kind == RSS_DDC_CHARACTERIZATION_INTERACTION_NONE);
    assert(first.semantic_id[0] == '\0');
    assert(first.risk == RSS_DDC_CHARACTERIZATION_INTERACTION_RISK_NONE);
    assert(!first.may_set);
    assert(!first.restore_required);
    assert(!first.restore_available);
    assert(first.option_count == 0);
    assert(rss_ddc_characterization_interaction(characterization, &second) == RSS_DDC_OK);
    assert(memcmp(&first, &second, sizeof(first)) == 0);
    assert(rss_ddc_characterization_stage(characterization) == stage);
    assert(rss_ddc_characterization_discovery_performed(characterization) == discovery);
    assert(rss_ddc_characterization_quick_attempted(characterization) == quick);
    assert(rss_ddc_characterization_extended_attempted(characterization) == extended);
    assert_write_authority_unchanged(characterization, authorized, route_count);
}

static void test_interaction_automatic_default_has_none(void) {
    Slice7Harness harness = slice7_harness(lg_hdr_qhd_display());
    RSSDDCProfileStore *store = rss_ddc_profile_store_create();
    RSSDDCCharacterization *result = NULL;
    assert(store != NULL);
    ...
    harness.mccs_raw = "vcp(10 12 15 60)";
    assert(driver_run(&harness, store, NULL, &result) == RSS_DDC_OK);
    assert_no_pending_interaction(result, RSS_DDC_CHARACTERIZATION_ACTION_COMPLETE);
    assert(harness.set_calls == 0);
    rss_ddc_characterization_destroy(result);
    rss_ddc_profile_store_destroy(store);
}

static void test_interaction_inspect_has_none(void) {
    Slice7Harness harness = slice7_harness(lg_hdr_qhd_display());
    RSSDDCProfileStore *store = rss_ddc_profile_store_create();
    RSSDDCCharacterization *result = NULL;
    RSSDDCCharacterizationOps ops = slice7_ops(&harness);
...
static void test_interaction_complete_short_circuit_has_none(void) {
    Slice7Harness harness = slice7_harness(slice2_display());
    RSSDDCProfileStore *store = store_from_pack(slice5_actionable_pack_dcpdp13());
    RSSDDCCharacterization *result = NULL;
    assert(driver_run(&harness, store, NULL, &result) == RSS_DDC_OK);
...
static void test_interaction_passive_has_none(void) {
    Slice7Harness harness = slice7_harness(slice2_display());
    RSSDDCCharacterizeOptions options = {.mode = RSS_DDC_CHARACTERIZE_MODE_PASSIVE};
    RSSDDCCharacterization *result = NULL;
    harness.mccs_raw = "vcp(10 12 15 60)";
...
static void test_interaction_default_extended_has_none(void) {
    Slice7Harness harness = slice7_harness(slice2_display());
    RSSDDCProfileStore *store = store_from_pack(slice2_profile_pack());
    RSSDDCCharacterization *result = NULL;
    harness.mccs_raw = "vcp(10 12 15 60)";
...
static void test_interaction_deep_has_none(void) {
    Slice7Harness harness = slice7_harness(slice2_display());
    RSSDDCProfileStore *store = store_from_pack(slice5_actionable_pack_dcpdp13());
    RSSDDCCharacterization *result = NULL;
    RSSDDCCharacterizeOptions options = {.mode = RSS_DDC_CHARACTERIZE_MODE_DEEP};
...
static void test_interaction_lifetime_and_submit_shell(void) {
    Slice7Harness harness = slice7_harness(slice2_display());
    RSSDDCCharacterization *result = NULL;
    RSSDDCCharacterizationOps ops = slice7_ops(&harness);
    RSSDDCCharacterizationInteraction copied;
    RSSDDCCharacterizationInteractionResult submission = {
        .kind = RSS_DDC_CHARACTERIZATION_INTERACTION_OPERATOR_SELECTION,
        .confirmed = true,
    };
    RSSDDCCharacterizationStage stage = RSS_DDC_CHARACTERIZATION_STAGE_NEW;
    bool authorized[128];
    size_t route_count = 0;
    (void)snprintf(submission.option_id, sizeof(submission.option_id), "%s", "hdmi-1");
    assert(rss_ddc_characterization_begin_with_ops(3, NULL, NULL, &ops, &result) == RSS_DDC_OK);
    assert_no_pending_interaction(result, RSS_DDC_CHARACTERIZATION_ACTION_PREPARE);
    assert(rss_ddc_characterization_next_interaction(NULL) == RSS_DDC_CHARACTERIZATION_INTERACTION_NONE);
    assert(rss_ddc_characterization_interaction(NULL, &copied) == RSS_DDC_ERROR_ARGUMENT);
    assert(rss_ddc_characterization_interaction(result, NULL) == RSS_DDC_ERROR_ARGUMENT);
    assert(rss_ddc_characterization_submit_interaction(NULL, &submission) == RSS_DDC_ERROR_ARGUMENT);
    assert(rss_ddc_characterization_submit_interaction(result, NULL) == RSS_DDC_ERROR_ARGUMENT);
    submission.kind = RSS_DDC_CHARACTERIZATION_INTERACTION_NONE;
    assert(rss_ddc_characterization_submit_interaction(result, &submission) == RSS_DDC_ERROR_ARGUMENT);
    submission.kind = RSS_DDC_CHARACTERIZATION_INTERACTION_VALIDATION_APPROVAL;
    stage = rss_ddc_characterization_stage(result);
    snapshot_write_authority(result, authorized, &route_count);
    assert(rss_ddc_characterization_submit_interaction(result, &submission) == RSS_DDC_ERROR_NOT_FOUND);
    assert(rss_ddc_characterization_stage(result) == stage);
    assert(rss_ddc_characterization_next_interaction(result) == RSS_DDC_CHARACTERIZATION_INTERACTION_NONE);
    assert(rss_ddc_characterization_next_action(result) == RSS_DDC_CHARACTERIZATION_ACTION_PREPARE);
    assert_write_authority_unchanged(result, authorized, route_count);
    assert(harness.set_calls == 0);
    rss_ddc_characterization_destroy(result);
}

static RSSDDCCharacterizeOptions input_goal_options(void) {
    RSSDDCCharacterizeOptions options = rss_ddc_default_characterize_options();
    (void)snprintf(options.semantic_goal, sizeof(options.semantic_goal), "%s", "inputs.switching");
    return options;
}

static const RSSDDCKnowledgeRoute *guided_discovery_route(const RSSDDCMonitorKnowledge *knowledge) {
    size_t index = 0;
    for (index = 0; index < rss_ddc_monitor_knowledge_route_count(knowledge); ++index) {
        ...
        if (route != NULL && strcmp(route->semantic_id, "inputs.switching") == 0 &&
            strcmp(route->provenance.source_id, "guided-discovery") == 0) {
            return route;
        }
    }
    return NULL;
...
static void test_goal_absent_preserves_automatic_complete(void) {
    Slice7Harness harness = slice7_harness(slice2_display());
    RSSDDCCharacterization *result = NULL;
    RSSDDCCharacterizeOptions options = rss_ddc_default_characterize_options();
    slice7_stable_quick(&harness, 42);
    harness.mccs_raw = "vcp(10 12 15 60)";
    slice6_set_stable(&harness.extended, 0x60, 18, 0x11);
    assert(options.semantic_goal[0] == '\0');
    assert(driver_run(&harness, NULL, &options, &result) == RSS_DDC_OK);
    assert(rss_ddc_characterization_semantic_goal(result) == NULL);
    assert(rss_ddc_characterization_next_action(result) == RSS_DDC_CHARACTERIZATION_ACTION_COMPLETE);
    assert(rss_ddc_characterization_next_interaction(result) == RSS_DDC_CHARACTERIZATION_INTERACTION_NONE);
    assert(harness.set_calls == 0);
    rss_ddc_characterization_destroy(result);
...
static void test_goal_is_stored_and_observable(void) {
    Slice7Harness harness = slice7_harness(slice2_display());
    RSSDDCCharacterization *result = NULL;
    RSSDDCCharacterizationOps ops = slice7_ops(&harness);
    RSSDDCCharacterizeOptions options = input_goal_options();
    (void)snprintf(options.semantic_goal, sizeof(options.semantic_goal), "%s", "input");
    assert(rss_ddc_characterization_begin_with_ops(3, NULL, &options, &ops, &result) == RSS_DDC_OK);
    assert(rss_ddc_characterization_semantic_goal(result) != NULL);
    assert(strcmp(rss_ddc_characterization_semantic_goal(result), "inputs.switching") == 0);
    rss_ddc_characterization_destroy(result);
}

static void test_goal_cannot_change_after_prepare(void) {
    Slice7Harness harness = slice7_harness(slice2_display());
    RSSDDCCharacterization *result = NULL;
    RSSDDCCharacterizationOps ops = slice7_ops(&harness);
    RSSDDCCharacterizeOptions options = input_goal_options();
    assert(rss_ddc_characterization_begin_with_ops(3, NULL, &options, &ops, &result) == RSS_DDC_OK);
    assert(rss_ddc_characterization_set_semantic_goal(result, "display.brightness") == RSS_DDC_OK);
    assert(strcmp(rss_ddc_characterization_semantic_goal(result), "display.brightness") == 0);
    assert(rss_ddc_characterization_run_next(result) == RSS_DDC_OK);
    assert(rss_ddc_characterization_stage(result) == RSS_DDC_CHARACTERIZATION_STAGE_IDENTITY);
    assert(rss_ddc_characterization_set_semantic_goal(result, "inputs.switching") ==
           RSS_DDC_ERROR_ARGUMENT);
    assert(strcmp(rss_ddc_characterization_semantic_goal(result), "display.brightness") == 0);
    rss_ddc_characterization_destroy(result);
}

static void test_known_authorized_input_skips_guided(void) {
    Slice7Harness harness = slice7_harness(lg_hdr_qhd_display());
    RSSDDCProfileStore *store = rss_ddc_profile_store_create();
    RSSDDCCharacterization *result = NULL;
    RSSDDCCharacterizeOptions options = input_goal_options();
    ...
    assert(rss_ddc_characterization_resolve(result, "inputs.switching", &resolution) == RSS_DDC_OK);
    assert(rss_ddc_monitor_knowledge_resolution_write_authorized(resolution));
    assert(guided_discovery_route(rss_ddc_characterization_knowledge(result)) == NULL);
    assert(harness.set_calls == 0);
    rss_ddc_monitor_knowledge_resolution_destroy(resolution);
    rss_ddc_characterization_destroy(result);
    rss_ddc_profile_store_destroy(store);
...
static void test_complete_authorized_profile_skips_guided(void) {
    Slice7Harness harness = slice7_harness(slice2_display());
    RSSDDCProfileStore *store = store_from_pack(slice5_actionable_pack_dcpdp13());
    RSSDDCCharacterization *result = NULL;
    RSSDDCCharacterizeOptions options = input_goal_options();
...
static void test_inspect_with_goal_does_not_queue_guided(void) {
    Slice7Harness harness = slice7_harness(odyssey_g75f_display());
    RSSDDCCharacterization *result = NULL;
    RSSDDCCharacterizationOps ops = slice7_ops(&harness);
    RSSDDCCharacterizeOptions options = input_goal_options();
    assert(rss_ddc_characterization_inspect_with_ops(1, NULL, &options, &ops, &result) == RSS_DDC_OK);
    assert(strcmp(rss_ddc_characterization_semantic_goal(result), "inputs.switching") == 0);
    assert(rss_ddc_characterization_next_interaction(result) == RSS_DDC_CHARACTERIZATION_INTERACTION_NONE);
    assert(rss_ddc_characterization_next_action(result) == RSS_DDC_CHARACTERIZATION_ACTION_COMPLETE);
    assert(harness.quick_calls == 0);
...
static void test_unknown_without_input_observation_does_not_invent_guided(void) {
    Slice7Harness harness = slice7_harness(odyssey_g75f_display());
    RSSDDCCharacterization *result = NULL;
    RSSDDCCharacterizeOptions options = input_goal_options();
    slice7_stable_quick(&harness, 42);
...
static RSSDDCCharacterization *run_unknown_input_goal(Slice7Harness *harness) {
    RSSDDCCharacterization *result = NULL;
    RSSDDCCharacterizeOptions options = input_goal_options();
    harness->mccs_raw = "vcp(10 12 15 60)";
    slice7_stable_quick(harness, 42);
...
static void test_unknown_with_observed_input_queues_guided(void) {
    Slice7Harness harness = slice7_harness(slice2_display());
    RSSDDCCharacterization *result = run_unknown_input_goal(&harness);
    RSSDDCCharacterizationInteraction interaction = {0};
    assert(rss_ddc_characterization_next_action(result) ==
           RSS_DDC_CHARACTERIZATION_ACTION_WAIT_FOR_INTERACTION);
    assert(rss_ddc_characterization_stage(result) == RSS_DDC_CHARACTERIZATION_STAGE_INTERACTION);
    assert(rss_ddc_characterization_next_interaction(result) ==
           RSS_DDC_CHARACTERIZATION_INTERACTION_OPERATOR_SELECTION);
    assert(rss_ddc_characterization_interaction(result, &interaction) == RSS_DDC_OK);
    assert(strcmp(interaction.semantic_id, "inputs.switching") == 0);
    assert(interaction.risk == RSS_DDC_CHARACTERIZATION_INTERACTION_RISK_OBSERVE);
    assert(!interaction.may_set);
    assert(!interaction.restore_required);
    assert(interaction.option_count == 0);
    assert(harness.set_calls == 0);
    rss_ddc_characterization_destroy(result);
}

static void test_guided_submit_records_non_authoritative_evidence(void) {
    Slice7Harness harness = slice7_harness(slice2_display());
    RSSDDCCharacterization *result = run_unknown_input_goal(&harness);
    RSSDDCCharacterizationInteractionResult submission = {
        .kind = RSS_DDC_CHARACTERIZATION_INTERACTION_OPERATOR_SELECTION,
    };
    RSSDDCCharacterizationInteractionResult malformed = {
        .kind = RSS_DDC_CHARACTERIZATION_INTERACTION_OPERATOR_CONFIRMATION,
    };
    RSSDDCCharacterizationInteractionResult empty = {
        .kind = RSS_DDC_CHARACTERIZATION_INTERACTION_OPERATOR_SELECTION,
    };
    const RSSDDCKnowledgeRoute *guided = NULL;
    RSSDDCMonitorKnowledgeResolution *resolution = NULL;
    size_t discovered_before = 0;
    (void)snprintf(submission.option_id, sizeof(submission.option_id), "%s", "osd-input-a");
    (void)snprintf(malformed.option_id, sizeof(malformed.option_id), "%s", "osd-input-a");
    discovered_before =
        rss_ddc_monitor_knowledge_route_count(rss_ddc_characterization_discovered_knowledge(result));
    assert(rss_ddc_characterization_submit_interaction(result, &malformed) == RSS_DDC_ERROR_ARGUMENT);
    assert(rss_ddc_characterization_next_action(result) ==
           RSS_DDC_CHARACTERIZATION_ACTION_WAIT_FOR_INTERACTION);
    assert(rss_ddc_characterization_submit_interaction(result, &empty) == RSS_DDC_ERROR_ARGUMENT);
    assert(rss_ddc_characterization_submit_interaction(result, &submission) == RSS_DDC_OK);
    assert(rss_ddc_characterization_next_action(result) == RSS_DDC_CHARACTERIZATION_ACTION_COMPLETE);
    assert(rss_ddc_characterization_next_interaction(result) == RSS_DDC_CHARACTERIZATION_INTERACTION_NONE);
    assert(rss_ddc_characterization_stage(result) == RSS_DDC_CHARACTERIZATION_STAGE_COMPLETE);
    guided = guided_discovery_route(rss_ddc_characterization_knowledge(result));
    assert(guided != NULL);
    assert(guided->provenance.fact_kind == RSS_DDC_KNOWLEDGE_FACT_INFERRED);
    assert(guided->provenance.confidence == RSS_DDC_PROFILE_CONFIDENCE_CORRELATED);
    assert(guided->provenance.source == RSS_DDC_PROFILE_SOURCE_RESEARCH);
    assert(strcmp(guided->value.string_value, "osd-input-a") == 0);
    assert(!guided->writable);
    assert(!guided->write_authorized);
    assert(guided_discovery_route(rss_ddc_characterization_discovered_knowledge(result)) == NULL);
    assert(rss_ddc_monitor_knowledge_route_count(rss_ddc_characterization_discovered_knowledge(result)) ==
           discovered_before);
    assert(rss_ddc_characterization_resolve(result, "inputs.switching", &resolution) == RSS_DDC_OK);
    assert(!rss_ddc_monitor_knowledge_resolution_write_authorized(resolution));
    assert(rss_ddc_characterization_submit_interaction(result, &submission) == RSS_DDC_ERROR_NOT_FOUND);
...
static void test_conflict_goal_does_not_queue_guided(void) {
    Slice7Harness harness = slice7_harness(slice2_display());
    RSSDDCProfileStore *store = conflict_store();
    RSSDDCCharacterization *result = NULL;
    RSSDDCCharacterizeOptions options = input_goal_options();
...
int main(void) {
    ...
    test_profile_free_lg_does_not_acquire_lg_alt();
    test_builtin_lg_profile_adds_lg_alt_authority();
    test_odyssey_gains_no_model_specific_authority();
    test_observed_vcp60_never_creates_lg_alt();
    test_complete_match_skips_default_probing();
    test_complete_match_skips_passive_mccs();
    test_partial_match_does_not_skip_default();
    test_deep_discovers_with_complete_profile();
    ...
    test_interaction_automatic_default_has_none();
    test_interaction_inspect_has_none();
    test_interaction_complete_short_circuit_has_none();
    test_interaction_passive_has_none();
    test_interaction_default_extended_has_none();
    test_interaction_deep_has_none();
    test_interaction_lifetime_and_submit_shell();
    test_goal_absent_preserves_automatic_complete();
    test_goal_is_stored_and_observable();
    test_goal_cannot_change_after_prepare();
    test_known_authorized_input_skips_guided();
    test_complete_authorized_profile_skips_guided();
    test_inspect_with_goal_does_not_queue_guided();
    test_unknown_without_input_observation_does_not_invent_guided();
    test_unknown_with_observed_input_queues_guided();
    test_guided_submit_records_non_authoritative_evidence();
    test_conflict_goal_does_not_queue_guided();
