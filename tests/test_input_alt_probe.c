...
static RSSDDCError write_i2c(void *context, void *service, uint32_t chip, uint32_t data, const uint8_t *payload, size_t length) {
    ...
    memcpy(observed_payload, payload, length); return RSS_DDC_OK;
}
...
static void reset(void) { construct_calls = delay_calls = write_calls = release_calls = 0; observed_chip = observed_data = 0; observed_payload_length = 0; memset(observed_payload, 0, sizeof(observed_payload)); }
...
int main(void) {
    ...
    reset();
    assert(rss_ddc_run_dcpdp13_input_alt_probe(RSS_DDC_PROVIDER_DCPDP_SERVICE, RSS_DDC_INPUT_ALT_PROBE_CONVENTIONAL, 0x11, &callbacks) == RSS_DDC_ERROR_UNSUPPORTED_PROVIDER);
