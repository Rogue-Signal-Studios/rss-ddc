...
static unsigned int dp_lg_alt_calls;
...
RSSDDCError rss_macos_dp_set_lg_alt_input(RSSMacOSBinding *binding, uint16_t value,
                                          const RSSDDCDiagnostics *diagnostics) {
    (void)binding; (void)value; (void)diagnostics;
    ++dp_lg_alt_calls;
    return RSS_DDC_OK;
}
...
int main(void) {
    ...
    assert(rss_macos_provider_set_vcp(&selected_dp, 0x10, 18, NULL) == RSS_DDC_OK);
    assert(rss_macos_provider_set_lg_alt_input(&selected_dp, 0x90, NULL) == RSS_DDC_OK);
    assert(dp_get_calls == 1 && ps190_get_calls == 0 && dp_set_calls == 1 && dp_lg_alt_calls == 1 && ps190_set_calls == 0);
    RSSDDCMCCSCapabilities capabilities = {};
    assert(rss_macos_provider_get_mccs_capabilities(&selected_dp, &capabilities, NULL) == RSS_DDC_OK);
    assert(dp_mccs_calls == 1 && dp_get_calls == 1 && dp_set_calls == 1);
    ...
    assert(rss_macos_provider_set_lg_alt_input(&selected_dcpdpservice, 0x90, NULL) == RSS_DDC_ERROR_UNSUPPORTED_CAPABILITY);
    ...
    assert(rss_macos_provider_set_lg_alt_input(&selected_ps190, 0x90, NULL) == RSS_DDC_ERROR_UNSUPPORTED_CAPABILITY);
    ...
    assert(dp_mccs_calls == 1 && ps190_get_calls == 1 && ps190_set_calls == 1);
    ...
    assert(rss_macos_provider_set_lg_alt_input(&selected_unknown, 0x90, NULL) == RSS_DDC_ERROR_UNSUPPORTED_PROVIDER);
    assert(dp_set_calls == 2 && dp_lg_alt_calls == 1 && ps190_set_calls == 1 && ps190_dpcd_calls == 1 && dp_dpcd_calls == 2);
