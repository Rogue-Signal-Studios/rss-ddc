...
static const RSSDDCLGAltInputCallbacks callbacks = {.construct = construct, .prewrite_delay = delay, .write_i2c = write_i2c, .release = release};
...
int main(void) {
    ...
    const uint8_t expected_hdmi_1[] = {0x84, 0x03, 0xf4, 0x00, 0x90, 0xdd};
    assert(plan.chip == 0x37 && plan.data == 0x50 && plan.payload_length == 6 && plan.write_count == 2 && plan.prewrite_delay_us == 10000);
    assert(memcmp(plan.payload, expected_hdmi_1, sizeof(expected_hdmi_1)) == 0);
    assert(rss_ddc_prepare_lg_alt_input(0x91, &plan) == RSS_DDC_OK);
    const uint8_t expected_hdmi_2[] = {0x84, 0x03, 0xf4, 0x00, 0x91, 0xdc};
    assert(memcmp(plan.payload, expected_hdmi_2, sizeof(expected_hdmi_2)) == 0);
    assert(rss_ddc_prepare_lg_alt_input(0xd0, &plan) == RSS_DDC_OK);
    const uint8_t expected_displayport_1[] = {0x84, 0x03, 0xf4, 0x00, 0xd0, 0x9d};
    assert(memcmp(plan.payload, expected_displayport_1, sizeof(expected_displayport_1)) == 0);
    assert(rss_ddc_prepare_lg_alt_input(0x100, &plan) == RSS_DDC_ERROR_ARGUMENT);
    ...
    assert(memcmp(observed_payload, expected_hdmi_1, sizeof(expected_hdmi_1)) == 0);
    /* This write-only runner has no GET, verify, restore, or fallback callback. */
    reset();
