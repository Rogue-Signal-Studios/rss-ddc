...
protocol-standard; the IOAV chip/data/delay contract is conventional-DP
evidence shared by DCPDP13 and hardware-validated DCPDPService GET. For
`DCPDPService`, the same `dcpav-service-epic` object is used for GET.
`DCPDPServiceProxy` is a sibling on `dcpdp-service-epic` and is not used.
`DCPDPDeviceProxy` is validated for DPCD only, not DDC GET.
