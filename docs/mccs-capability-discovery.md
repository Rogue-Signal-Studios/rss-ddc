...
parsers/builders for individual DDC/CI capability packets. It does **not** yet
issue MCCS Capabilities Request packets at runtime, advertise a provider
capability for them, or provide an `rss-ddc capabilities` command. This is
deliberate: the current macOS provider evidence validates a fixed 11-byte Get
VCP read, not variable-length reads plus the sequential transactions required
for capability retrieval.
...
capability-string bytes. Thus a full DDC/CI reply is at most 39 bytes. The
...
then a fixed 11-byte read. `AppleDCPPS190` has a different hardware-validated
raw GET shape: inline `0x51`, `UINT32_MAX` no-offset, 50 ms, then a fixed
11-byte read. That is enough evidence to keep their existing GET VCP paths,
but not enough to claim that their private IOAV implementations will return a
variable-size capability reply correctly or safely over many sequential reads.

No validation harness is therefore justified yet. The next required evidence
is a narrowly scoped, user-run raw IOAV experiment for each intended provider:
one selected display, one `F3` request at offset zero, a 39-byte read buffer,
full byte/IOReturn capture, strict `E3` validation, and no retry/fallback. Only
after that proves the first transaction should a second, separately bounded
sequential-fragment experiment be considered. MCDP remains out of scope.
