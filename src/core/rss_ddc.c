...
RSSDDCError rss_macos_probe_mccs_capabilities(uint32_t list_index, const RSSDDCDiagnostics *diagnostics) {
    ...
    if (error != RSS_DDC_OK) {
        ...
        char message[192] = {};
        snprintf(message, sizeof(message),
                 "operation=ProbeMCCSCapabilities provider=%s status=unsupported; DCPDP13Service only",
                 rss_ddc_provider_string(binding.display.provider));
        rss_macos_diagnostic(diagnostics, message);
        error = RSS_DDC_ERROR_UNSUPPORTED_CAPABILITY;
    } else {
        diagnostic_binding(&binding, diagnostics);
        error = rss_macos_dcpdp13_probe_mccs_capabilities(&binding, diagnostics);
    }
    rss_macos_release_binding(&binding);
    return error;
}
...
/* CLI-only single-offset follow-up to the recorded LG first-frame result. */
RSSDDCError rss_macos_probe_mccs_capabilities_next_fragment(uint32_t list_index,
    ...
    if (error != RSS_DDC_OK) {
        ...
        char message[192] = {};
        snprintf(message, sizeof(message),
                 "operation=ProbeMCCSCapabilitiesNextFragment provider=%s status=unsupported; DCPDP13Service only",
                 rss_ddc_provider_string(binding.display.provider));
        rss_macos_diagnostic(diagnostics, message);
        error = RSS_DDC_ERROR_UNSUPPORTED_CAPABILITY;
    } else {
        diagnostic_binding(&binding, diagnostics);
        error = rss_macos_dcpdp13_probe_mccs_capabilities_next_fragment(&binding, diagnostics);
    }
    rss_macos_release_binding(&binding);
    return error;
}

/* CLI-only bounded developer validation; intentionally absent from the installed public header. */
RSSDDCError rss_macos_probe_mccs_capabilities_full(uint32_t list_index, const RSSDDCDiagnostics *diagnostics) {
    ...
    if (error != RSS_DDC_OK) {
        ...
        char message[192] = {};
        snprintf(message, sizeof(message),
                 "operation=ProbeMCCSCapabilitiesFull provider=%s status=unsupported; DCPDP13Service only",
                 rss_ddc_provider_string(binding.display.provider));
        rss_macos_diagnostic(diagnostics, message);
        error = RSS_DDC_ERROR_UNSUPPORTED_CAPABILITY;
    } else {
        diagnostic_binding(&binding, diagnostics);
        error = rss_macos_dcpdp13_probe_mccs_capabilities_full(&binding, diagnostics);
    }
    rss_macos_release_binding(&binding);
    return error;
}
