...
bool rss_ddc_dp_device_proxy_matches(bool external, const char *epic_name, const char *candidate_role,
    ...
    return external && epic_name != NULL && candidate_role != NULL && selected_service_role != NULL &&
        strcmp(epic_name, "dcpdp-device-epic") == 0 && candidate_role[0] != '\0' &&
        strcmp(candidate_role, selected_service_role) == 0;
}
