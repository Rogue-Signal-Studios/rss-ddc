...
static RSSDDCError lg_alt_write(void *opaque, void *opaque_service, uint32_t chip, uint32_t data,
    ...
    ++context->write_index;
    char message[160] = {};
    snprintf(message, sizeof(message), "LG_ALT write=%u/%u chip=0x%02x data=0x%02x length=%zu pre-delay=10ms IOReturn=0x%08x",
...
RSSDDCError rss_macos_dp_set_lg_alt_input(RSSMacOSBinding *binding, uint16_t value,
    ...
    char message[224] = {};
    snprintf(message, sizeof(message), "operation=SetInput method=LG_ALT target=LG-HDR-QHD/DCPDP13Service/DCPEXT0 value=0x%02x writes=%u no-get=yes no-verify=yes no-retry=yes no-fallback=yes",
